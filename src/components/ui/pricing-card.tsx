...
export function PricingCard({ tier, paymentFrequency }: PricingCardProps) {
  ...
  return (
    <div
      className={`relative flex flex-col rounded-2xl border bg-white p-6 shadow-md transition-all duration-300 ${
    ...
    >
      ...
      <h3 className="text-2xl font-bold mb-2 text-[#0A090C]">{tier.name}</h3>
      <div className="flex items-end mb-4">
        <span className="text-4xl font-bold text-[#0A090C]">
      ...
      <p className="text-[#0A090C] opacity-70 mb-6">{tier.description}</p>
      <ul className="space-y-3 mb-8">
        {tier.features.map((feature, idx) => (
          <li key={idx} className="flex items-center">
            <Check className="h-5 w-5 text-[#009A53] shrink-0 mr-2" />
