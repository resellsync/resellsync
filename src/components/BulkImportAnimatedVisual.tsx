...
export default function BulkImportAnimatedVisual() {
  ...
  return <div className="relative w-[95vw] max-w-[480px] md:max-w-[580px] mx-auto h-[325px] md:h-[380px] bg-gradient-to-br from-brand-blue/5 to-brand-teal/10 flex flex-col gap-8 p-4 shadow-[0_1px_6px_0_rgba(10,93,117,0.05)] overflow-hidden rounded-xl bg-white px-0 py-0">
        ...
        <div className="bg-white rounded-xl shadow-lg w-full overflow-hidden border border-gray-200 animate-fade-in">
          ...
          <div className="px-4">
            <table className="w-full text-xs">
              ...
              <tbody>
                {dummyRows.slice(0, shownRows).map((row, i) => <tr key={row.imei} className={`transition-all duration-500 ${i % 2 === 1 ? "bg-gray-50" : ""} ${shownRows > i ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-6"}`} style={{
